...
class UserManager extends  AppBase{
    ...
    try {
      switch (provider) {
        case SIGNIN_METHODS.CustomToken:
          ...
          _token = codec.encode(
              new FirebaseToken(authData)); //todo  get real auth token here
          ...
          if (auth["uid"] != null) {
            ...
            throw new AppError(
                actionType: reg ? ActionsTypes.register : ActionsTypes.login,
                payload: aUser,
                message: "error occurred during signin/register");
          }
        ...
        default:
          throw (new AppError(
              actionType: reg ? ActionsTypes.register : ActionsTypes.login,
              payload: aUser,
              message: "un recognized SIGNIN_METHOD"));
      }
    } catch (e, st) {
  ...
  Future<bool> givePermission(permission perm) async {
    ...
    try {
      if (!(aUser.permissions.containsKey(perm.id) &&
          aUser.permissions[perm.id].status)) {
        var permRef =
            _usersRef.child(aUser.uid).child("permissions").child(perm.id);
        await permRef.set({"id": perm.id, "name": perm.name, "status": true});
        var permVal = await permRef.get();
        if (permVal != null && permVal["status"]) {
          var permInfo = (new permission()
            ..id = permVal["id"]
            ..name = permVal["name"]);
          store.dispatch(
              new Action(type: ActionsTypes.givePermission, data: permInfo));
        } else {
          throw (new AppError(
              actionType: ActionsTypes.givePermission,
              payload: perm,
              message: "failed to give permissions"));
        }

        store.dispatch(
            new Action(type: ActionsTypes.givePermission, data: perm));
      }
  ...
  Future<bool> revokeUserPermission(revokePermission perm) async {
    ...
    try {
      if (!(aUser.permissions.containsKey(perm.id) &&
          aUser.permissions[perm.id].status == false)) {
        var permRef =
            _usersRef.child(aUser.uid).child("permissions").child(perm.id);
        await permRef.set({"id": perm.id, "name": perm.name, "status": false});
        var permVal = await permRef.get();
        if (permVal != null && permVal["status"] == false) {
          ...
          throw (new AppError(
              actionType: ActionsTypes.revokePermission,
              payload: perm,
              message: "failed to revoke permission"));
        }

        store.dispatch(
            new Action(type: ActionsTypes.revokePermission, data: perm));
      }
  ...
  Future<bool> followUser(follow info) async {
    ...
    try {
      if (!(aUser.following.containsKey(info.uid) &&
          aUser.following[info.uid].status)) {
        var followRef =
            _usersRef.child(aUser.uid).child("following").child(info.uid);
        await followRef.set({
          "uid": info.uid,
        ...
        if (followVal != null && followVal["status"]) {
          ...
          store.dispatch(
              new Action(type: ActionsTypes.followUser, data: followInfo));
        } else {
          throw (new AppError(
              actionType: ActionsTypes.followUser,
              payload: info,
      ...
      if ((aUser.following.containsKey(info.uid) &&
        ...
        if (followVal != null && (followVal["status"] == false)) {
          unfollow unfollowInfo = (new unfollow()
            ...
            ..userName = followVal["userName"]);
          store.dispatch(
              new Action(type: ActionsTypes.unfollowUser, data: unfollowInfo));
        } else {
          throw (new AppError(
              actionType: ActionsTypes.followUser,
              payload: info,
      ...
      if (ref.auth != null || aUser != null) {
        ...
        if (ref.auth == null) {
          ...
          throw (new AppError(
              ...
              payload: store.state.currentUser,
