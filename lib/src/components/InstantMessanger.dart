...
class InstantMessanger {
  ...
  Future<Chat> createChat(Chat chat) async {
    ...
    try {
      ...
      if (chatVal == null) {
        ...
        if (chatVal == null) {
          throw (new AppError(
            actionType: ActionsTypes.createChat,
  ...
  Future<bool> deleteChat(Chat chat) async {
    try {
      ...
      if (deletedChat != null) {
        throw new AppError(
            actionType: ActionsTypes.deleteChat,
            payload: chat,
  ...
  Future<bool> sendMessage(Message msg) async {
    ...
    try {
 	...
 	if(msgData==null){ }else{
      ...
      if (msgData == null) {
        throw new AppError(
            actionType: ActionsTypes.sendMessage,
            payload: msg,
            message: "failed to send message to chat ${msg.chatId}");
      }
  ...
  Future<bool> deleteMessage(Message msg) async {
    try {
      ...
      if (deletedMsg != null) {
        throw new AppError(
            actionType: ActionsTypes.deleteMessage,
            payload: msg,
  ...
  Future<bool> addParticipant(Participate participation) async {
    ...
    try {
      ...
      if (participantInfo != null) {
        await _chatRef
            .child(participation.chatId)
            .child("participants")
            .child(participation.user.uid)
            .set({
        ...
        throw new AppError(
            actionType: ActionsTypes.addParticipant,
            payload: participation,
            message: "chat not found ");
      }
      participantInfo = await _chatRef
          .child(participation.chatId)
          .child("participants")
          .child(participation.user.uid)
          .get();
      ...
      store.dispatch(
          new Action(type: ActionsTypes.addParticipant, data: _participation));
      return true;
    } catch (e, st) {
  ...
  Future<bool> removeParticipant(Participate participation) async {
    ...
    try {
      ...
      if (participantInfo != null) {
        ...
        throw new AppError(
            actionType: ActionsTypes.removeParticipant,
            payload: participation,
            message: "chat not found ");
      }
      participantInfo = await _chatRef
      ...
      store.dispatch(new Action(
          type: ActionsTypes.removeParticipant, data: _participation));
      return true;
    } catch (e, st) {
